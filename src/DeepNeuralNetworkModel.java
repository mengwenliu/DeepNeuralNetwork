...
//	}
	...
	public void train(SimpleMatrix trainAllX, SimpleMatrix trainAllY) {
		...
		for (int i=0; i < this.maxIter; i++) {
			...
			ArrayList<SimpleMatrix> unRegGradients = 
					new ArrayList<SimpleMatrix>(getUnRegGradients(allX, theta1, theta2, inputLayerSize, hiddenLayerSize, m));
...
//		System.out.println("reg2: " + reg2.toString());
	...
	private ArrayList<SimpleMatrix> getUnRegGradients(SimpleMatrix allX, SimpleMatrix theta1, SimpleMatrix theta2, int inputLayerSize, int hiddenLayerSize, int m) {
...
//		printMatrixDimension(theta1, "theta1");
		...
		// 
		SimpleMatrix gradAllX = theta1.transpose().mult(delta2);
		//printMatrixDimension(allGradAllX, "allGradAllX");

		gradAllX = gradAllX.extractMatrix(1, gradAllX.numRows(), 0, gradAllX.numCols());
		//printMatrixDimension(allGradAllX, "allGradAllX");
...
//		printMatrixDimension(gradAllX, "gradAllX");
		
		
		gradients.add(gradTheta1);
		gradients.add(gradTheta2);
		gradients.add(gradAllX);
	...
	private ArrayList<SimpleMatrix> getRegGradients(ArrayList<SimpleMatrix> unRegGradients, SimpleMatrix theta1, SimpleMatrix theta2, int m, double lambda) {
		...
		for (int i=0; i < unRegGradients.size()-1; i++) {
		...
		//add gradients for word embeddings
		regGradients.add(unRegGradients.get(unRegGradients.size()-1));
		return regGradients;
